...
const respond = async (request) => {
  ...
  return {
    ...
    body:
      "svelte-add-graphql doesn't support multipart responses or event streams",
    headers: {},
    status: 501,
  };
};

export const del = ({ body, query }, { headers }) =>
  respond({ body, headers, method: "DELETE", query });
export const get = ({ body, query }, { headers }) =>
  respond({ body, headers, method: "GET", query });
export const head = ({ body, query }, { headers }) =>
  respond({ body, headers, method: "HEAD", query });
export const post = ({ body, query }, { headers }) =>
  respond({ body, headers, method: "POST", query });
export const put = ({ body, query }, { headers }) =>
